...
const About = () => {
  ...
  return (
    <section className="py-14 font-syne bg-yellowish">
      <div className="max-w-screen-xl mx-auto px-4 text-center md:px-8">
        <div className="max-w-xl mx-auto">
          <h3 className="text-main text-3xl font-semibold sm:text-4xl">
            Meet our team
          </h3>
          ...
          <ul className="grid gap-8 sm:grid-cols-2 md:grid-cols-3">
            {
              team.map((item, idx) => (
                <li key={idx}>
                  <div className="w-28 h-28 mx-auto ring-4 rounded-full ring-main ">
                    <img
                      src={item.avatar}
                  ...
                  <div className="mt-2">
                    <h4 className="text-main text-[30px] font-[800] underline sm:text-lg">{item.name}</h4>
                    <p className="text-main">{item.title}</p>
                    <p className="text-main mt-2">{item.desc}</p>
                    <div className="mt-4 flex justify-center gap-4 text-navbarColor">
                      <a href={item.twitter}>
